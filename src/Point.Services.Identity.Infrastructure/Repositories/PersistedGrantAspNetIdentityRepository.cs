    ...
    where TUserToken : IdentityUserToken<TKey>
{
    ...
    public virtual Task<PagedList<PersistedGrantDataView>> GetPersistedGrantsByUsersAsync(string search, int page = 1, int pageSize = 10)
    {
        return Task.Run(() =>
        {
            ...
            var persistedGrantByUsers =
                   (from pe in PersistedGrantDbContext.PersistedGrants.ToList()
            ...
            var persistedGrantsData = persistedGrantDataViews
                .AsQueryable()
    ...
    public virtual async Task<PagedList<PersistedGrant>> GetPersistedGrantsByUserAsync(string subjectId, int page = 1, int pageSize = 10)
    {
        ...
        var persistedGrantsData = await PersistedGrantDbContext.PersistedGrants
        ...
        var persistedGrantsCount = await PersistedGrantDbContext.PersistedGrants
            .Where(x => x.SubjectId == subjectId)
            .CountAsync();
